...
**Realize/skip geri alma (v0.3.1):** Yanlış işaretlenen periyodik dönemleri düzeltmek için `GET /planned-expenses/{id}/periods` her dönemi `pending|realized(expense_id)|skipped(skip_id)` döner (geçmiş+güncel, en yeni üstte); `POST /planned-expenses/{id}/unrealize {year,month}` realize'ı geri alır (bağlı expense silinir, idempotent `removed:0/1`); skip geri alma mevcut `DELETE /recurring/skips/{id}` ile. Frontend `PlannedList` satırında "Dönemler" butonu → `PlannedPeriodsModal` (dönem durumu + toggle: pending→Gerçekleşti/Gerçekleşmeyecek, realized/skipped→Geri al).
