...
**Manuel kripto (API'siz borsalar):** API erişimi olmayan borsalar (BinanceTR, iCrypex, BTCTurk, Paribu, Bybit, KuCoin, Bitget vb.) için kullanıcının manuel kayıt yapabildiği `manual_crypto_holdings` tablosu. Endpoint prefix `/manual-crypto`; CRUD + Excel import/export + anlık fiyatla zenginleştirilmiş listeleme. **`price_source` 4 mod desteği:** `'auto'` (Binance USDT + CoinGecko fallback — varsayılan), `'manual'` (kullanıcının `manual_unit_price_tl` alanı), `'gold_gram'`/`'silver_gram'` (commodity service'ten anlık altın/gümüş gr fiyatı — örn. iCrypex'in XAGX'i `silver_gram`'a bağlanır). Snapshot entegrasyonu `services/snapshot.py::_gather_manual_crypto_assets()` — `asset_type="crypto"`, `provider="manual:{exchange}"`. Manual/gold/silver kayıtlar için fiyat doğrudan asset'e enjekte edilir; auto kayıtlar ortak enrichment loop'unda yakalanır. Eksik fiyat (manual boş, metal fiyatı çekilemedi vb.) durumunda snapshot health `issues` listesine eklenip popup'ta gösterilir.
