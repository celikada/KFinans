...
│   │   ├── scheduler.py           # APScheduler — 5 cron: Pazar 23:00 snapshot, 03:00 token cleanup, 04:00 hard-delete, 04:30 audit purge, 09:00 push ödeme hatırlatma
...
**Scheduler (5 cron job, Europe/Istanbul):**
...
- Her gün 09:00 — `_push_due_payments_job` ödemesi ≤5 gün kalan kredi kartı borçları için abonelere Web Push gönderir (PUSH-001, v0.5.0).
...
- **Migration head:** `d0e1f2a3b4c5` (push_subscriptions, v0.5.0; down_revision `c9d0e1f2a3b4`). v0.3.1/v0.3.2/v0.3.7/v0.4.0/v0.4.1 **migration İÇERMEZ** — yalnızca kod/endpoint/config. Yeni migration `down_revision = "d0e1f2a3b4c5"`. Zincir: `f3a4b5c6d7e8` (kredi ledger) → `a7b8c9d0e1f2` (recurring_skips) → `b8c9d0e1f2a3` (sürüm bildirimi opt-in) → `c9d0e1f2a3b4` (çoklu para birimi) → `d0e1f2a3b4c5` (push_subscriptions, **head**).
