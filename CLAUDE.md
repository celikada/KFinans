...
**Kredi kartları (`credit_cards`):** Faz 3 finans modülünün ilk parçası. Kart tanımı (name, bank_name, last_4, credit_limit) + dönem ayarları (statement_day, payment_due_day) + **`current_period_debt`** (dönem içi henüz ekstreye düşmemiş tutar — kullanıcı manuel günceller). Ekstre + taksit tabloları sonraki adımda eklenecek. Endpoint prefix `/credit-cards` (CRUD + summary). Frontend `/dashboard/credit-cards` form + tablo. Dashboard "Kredi Kartları" kartı **Finans grubunun en üstünde** (`creditCards` ID'siyle DASHBOARD_CARDS'da). Çift sayım kuralı (Expense + Planned'da credit_card_id + is_paid) sonraki adımda devreye girecek.
