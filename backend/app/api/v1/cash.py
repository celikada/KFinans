...
async def _amount_to_tl(amount: Decimal, currency: str) -> Decimal:
    """Currency → TL dönüşüm (ortak currency util'e delege).

    FIN-007 (FAZ H): TCMB her doviz icin ayri kur (EUR, USD, GBP, CHF, JPY...);
    v0.3.0 ile ortak `services/currency.py` util'i kullanir (DRY). Davranis
    korunur: TRY→1:1, var→carp, yoksa→USD/TRY yaklasik fallback, hicbiri yoksa 0.
    """
    rates = await currency_svc.fetch_rates()
    return currency_svc.convert_to_tl(amount, currency, rates)
